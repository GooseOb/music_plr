...
pub const FLOATING_SEARCH_ID: Id = Id::new("floating_search_input");

pub(super) fn view_floating_search(player: &MusicPlayer) -> Element<'_, Message, AppTheme> {
    let fs = player
        .floating_search
        .as_ref()
        .expect("view_floating_search called without a floating search");
    let p = &player.app_theme.palette;

    let count = match player.floating_match_position() {
        Some(pos) => format!("{}/{}", pos, fs.matches.len()),
        None => format!("0/{}", fs.matches.len()),
    };

    let input = text_input("Find in list…", &fs.query)
        .id(FLOATING_SEARCH_ID)
        .on_input(Message::FloatingSearchInput)
        .padding([theme::SPACING_XS, theme::SPACING_SM]);

    let prev = icon_button(
        crate::icons::CHEVRON_UP_ICON,
        Message::FloatingSearchPrev,
        p,
    );
    let next = icon_button(
        crate::icons::CHEVRON_DOWN_ICON,
        Message::FloatingSearchNext,
        p,
    );

    let close = icon_button(crate::icons::CLOSE_ICON, Message::FloatingSearchClose, p);
