...
- **Artist page**: `ViewKind::Artist { id, name, source, page }` carries a serializable `ArtistPageState` (`providers/artist_page.rs`) - known per-provider artist ids, header, and one `ArtistSection` (selected provider + loading + content) per section. `fetch_artist_page(provider, id, kinds)` (`ArtistDataKind`: Header/Popular/Albums/Playlists/Related) requests only the needed pieces - YT answers everything in one call, SoundCloud runs just the requested endpoints concurrently with retries. `open_artist()` loads the source provider's full page plus the other streamable provider's Header only, in parallel; a section-picker switch loads just that kind. Results arrive via `BackendResult::ArtistPageLoaded` and merge only into requested sections currently selecting that provider; fetched pages accumulate per provider in `CachedArtistPage` (page + which kinds are known) so switching back is request-free. Popular tracks double as the view's track list.
